...
void build_graph(uint16_t** connections, size_t node_count,uint16_t connection_count) {
	size_t span = 1;
	int power = 0;
	while (span<node_count) {
		for (size_t start = 0; start < node_count; start++) {
			size_t end = start + span; 
			if (end < node_count && 
					connections[start][power] == (uint16_t)-1 && 
					connections[end][power] == (uint16_t)-1) {
				connections[start][power] =  end;
				connections[end][power] =  start;
			}
		}
		power++;
		span = pow(2,power);	
	...
	for (size_t i = 0; i < node_count; i++) {
		cout << i << ":\t";
		for(uint16_t j = 0; j < connection_count;j++) {	
			cout << (int)connections[i][j] << '\t';
		}
		cout << endl;
	}
}
...
int main(int argc, char* argv[]) {
  ...
  while(true) {
	...
	partial_nodesets[nonce] = partial_nodeset;
      ...
      {//Time to make the spanning graph
	qsort(partial_nodeset.nodes, total, sizeof(client), socket_sort);
	
	uint16_t connection_count = (uint16_t)ceil(log((double)total)/log(2.0));
	uint16_t** connections = (uint16_t**)calloc(total,sizeof(uint16_t*));	
	for (size_t i=0; i<total; i++) {
		connections[i] = (uint16_t*)calloc(connection_count,sizeof(uint16_t)); 	
		std::fill_n(connections[i], connection_count, (uint16_t)-1);
	}
	build_graph(connections,total,connection_count);

	for (uint16_t i = 0; i < total; i++) {
	    	fail_send(partial_nodeset.nodes[i].socket, &connection_count, sizeof(connection_count));
	    	fail_send(partial_nodeset.nodes[i].socket, &i, sizeof(i));
	}
	...
	for (size_t i = 0; i < total; i++) {
			...
			if (connections[i][j] >= 0) {
				fail_send(partial_nodeset.nodes[i].socket
					,&partial_nodeset.nodes[connections[i][j]].client_ip
					,sizeof(partial_nodeset.nodes[connections[i][j]].client_ip));
				fail_send(partial_nodeset.nodes[i].socket
					, &client_ports[connections[i][j]]
					, sizeof(client_ports[connections[i][j]]));
			} else {
				int bogus = -1;
				uint32_t bogus2 = -1;
			...
			fail_send(partial_nodeset.nodes[i].socket, &connections[i][j], sizeof(connections[i][j]));
		}		
	}
